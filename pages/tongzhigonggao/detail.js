...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.biaoti,
...
const {
data
} = await detail("tongzhigonggao",id)
this.setData({
payAuth:utils.isAuthFront('tongzhigonggao','支付')
})
...
const resUpdate = await update('tongzhigonggao', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','tongzhigonggao');
...
async reply() {
const detailList = this.data.detailList
const res = await update("tongzhigonggao", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
