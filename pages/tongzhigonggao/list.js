...
let res = {};
if(this.data.userid) {
res = await page(`tongzhigonggao`, searchForm);
} else {
res = await list(`tongzhigonggao`, searchForm);
}
...
wx.navigateTo({
url: `/pages/tongzhigonggao/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("tongzhigonggao",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/tongzhigonggao/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/tongzhigonggao/detail`
})
...
this.setData({
addAuth: utils.isAuth("tongzhigonggao", "新增") || utils.isAuthFront("tongzhigonggao", "新增"),
delAuth: utils.isAuth("tongzhigonggao", "删除") || utils.isAuthFront("tongzhigonggao", "删除"),
editAuth: utils.isAuth("tongzhigonggao", "修改") || utils.isAuthFront("tongzhigonggao", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("tongzhigonggao",obj)
    }else{
         resList= await list("tongzhigonggao",obj)
    }
