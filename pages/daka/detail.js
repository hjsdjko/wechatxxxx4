...
const {
data
} = await detail("daka",id)
this.setData({
payAuth:utils.isAuthFront('daka','支付')
})
...
const resUpdate = await update('daka', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','daka');
...
async reply() {
const detailList = this.data.detailList
const res = await update("daka", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
