...
this.getData()
 ...
 let obj = {};
this.setData({
        dakaleixingOptions : '上班,下班'.split(',')
})
this.setData({
    ...obj
})
...
dakaleixingChange(e){
this.setData({
        dakaleixing: e.currentTarget.dataset.item
})
},
...
if(this.data.dakaleixing){
    searchForm['dakaleixing'] = this.data.dakaleixing
}


let res = {};
if(this.data.userid) {
res = await page(`daka`, searchForm);
} else {
res = await list(`daka`, searchForm);
}
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("daka",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/daka/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("daka", "新增") || utils.isAuthFront("daka", "新增"),
delAuth: utils.isAuth("daka", "删除") || utils.isAuthFront("daka", "删除"),
editAuth: utils.isAuth("daka", "修改") || utils.isAuthFront("daka", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("daka",obj)
    }else{
         resList= await list("daka",obj)
    }
