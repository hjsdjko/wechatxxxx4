...
if(this.data.yuangongxingming){
    searchForm['yuangongxingming'] = '%' + this.data.yuangongxingming + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`kaoqinxinxi`, searchForm);
} else {
res = await list(`kaoqinxinxi`, searchForm);
}
...
wx.navigateTo({
url: `/pages/kaoqinxinxi/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("kaoqinxinxi",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/kaoqinxinxi/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("kaoqinxinxi", "新增") || utils.isAuthFront("kaoqinxinxi", "新增"),
delAuth: utils.isAuth("kaoqinxinxi", "删除") || utils.isAuthFront("kaoqinxinxi", "删除"),
editAuth: utils.isAuth("kaoqinxinxi", "修改") || utils.isAuthFront("kaoqinxinxi", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("kaoqinxinxi",obj)
    }else{
         resList= await list("kaoqinxinxi",obj)
    }
