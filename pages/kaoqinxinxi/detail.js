...
const {
data
} = await detail("kaoqinxinxi",id)
this.setData({
payAuth:utils.isAuthFront('kaoqinxinxi','支付')
})

this.setData({
goodname:data.yuangongzhanghao
})
...
const resUpdate = await update('kaoqinxinxi', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','kaoqinxinxi');
...
async reply() {
const detailList = this.data.detailList
const res = await update("kaoqinxinxi", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
