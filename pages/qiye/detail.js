...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.qiyemingcheng,
...
const {
data
} = await detail("qiye",id)
this.setData({
payAuth:utils.isAuthFront('qiye','支付')
})
...
this.setData({
detailList,
picture: detailList.qiyetupian.split(','),
})
...
const resUpdate = await update('qiye', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
data["qiyetupian"] = data.qiyetupian
data['buynumber']=1
wx.setStorageSync('payObject',data);
wx.setStorageSync('paytable','qiye');
...
async reply() {
const detailList = this.data.detailList
const res = await update("qiye", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
