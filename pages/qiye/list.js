...
if(this.data.qiyemingcheng){
    searchForm['qiyemingcheng'] = '%' + this.data.qiyemingcheng + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`qiye`, searchForm);
} else {
res = await list(`qiye`, searchForm);
}
...
item.qiyetupian = item.qiyetupian.split(",")[0];
        return item
    })
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("qiye",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/qiye/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("qiye", "新增") || utils.isAuthFront("qiye", "新增"),
delAuth: utils.isAuth("qiye", "删除") || utils.isAuthFront("qiye", "删除"),
editAuth: utils.isAuth("qiye", "修改") || utils.isAuthFront("qiye", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("qiye",obj)
    }else{
         resList= await list("qiye",obj)
    }
...
.map(item => {
item.qiyetupian? item.qiyetupian = item.qiyetupian.split(",")[0].replace('upload','flie'):""
return item;
});
