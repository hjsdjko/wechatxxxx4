...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.yuangongxingming,
...
const {
data
} = await detail("qingjiashenqing",id)
this.setData({
payAuth:utils.isAuthFront('qingjiashenqing','支付')
})
...
const resUpdate = await update('qingjiashenqing', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','qingjiashenqing');
...
async reply() {
const detailList = this.data.detailList
const res = await update("qingjiashenqing", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
