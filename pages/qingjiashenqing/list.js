...
this.getData()
 ...
 let obj = {};
this.setData({
        qingjialeixingOptions : '事假,病假,其他'.split(',')
})
this.setData({
    ...obj
})
...
qingjialeixingChange(e){
this.setData({
        qingjialeixing: e.currentTarget.dataset.item
})
},
...
if(this.data.biaoti){
    searchForm['biaoti'] = '%' + this.data.biaoti + '%'
}
if(this.data.qingjialeixing){
    searchForm['qingjialeixing'] = this.data.qingjialeixing
}


let res = {};
if(this.data.userid) {
res = await page(`qingjiashenqing`, searchForm);
} else {
res = await list(`qingjiashenqing`, searchForm);
}
...
wx.navigateTo({
url: `/pages/qingjiashenqing/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("qingjiashenqing",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/qingjiashenqing/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/qingjiashenqing/detail`
})
...
this.setData({
addAuth: utils.isAuth("qingjiashenqing", "新增") || utils.isAuthFront("qingjiashenqing", "新增"),
delAuth: utils.isAuth("qingjiashenqing", "删除") || utils.isAuthFront("qingjiashenqing", "删除"),
editAuth: utils.isAuth("qingjiashenqing", "修改") || utils.isAuthFront("qingjiashenqing", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("qingjiashenqing",obj)
    }else{
         resList= await list("qingjiashenqing",obj)
    }
