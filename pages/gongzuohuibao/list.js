...
if(this.data.wanchengqingkuang){
    searchForm['wanchengqingkuang'] = '%' + this.data.wanchengqingkuang + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`gongzuohuibao`, searchForm);
} else {
res = await list(`gongzuohuibao`, searchForm);
}
...
wx.navigateTo({
url: `/pages/gongzuohuibao/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("gongzuohuibao",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/gongzuohuibao/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/gongzuohuibao/detail`
})
...
this.setData({
addAuth: utils.isAuth("gongzuohuibao", "新增") || utils.isAuthFront("gongzuohuibao", "新增"),
delAuth: utils.isAuth("gongzuohuibao", "删除") || utils.isAuthFront("gongzuohuibao", "删除"),
editAuth: utils.isAuth("gongzuohuibao", "修改") || utils.isAuthFront("gongzuohuibao", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("gongzuohuibao",obj)
    }else{
         resList= await list("gongzuohuibao",obj)
    }
