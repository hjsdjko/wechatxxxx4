...
const {
data
} = await detail("gongzuohuibao",id)
this.setData({
payAuth:utils.isAuthFront('gongzuohuibao','支付')
})
...
const resUpdate = await update('gongzuohuibao', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','gongzuohuibao');
...
async reply() {
const detailList = this.data.detailList
const res = await update("gongzuohuibao", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
