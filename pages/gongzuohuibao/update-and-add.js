...
if(o=='huibaomingcheng'){
            this.setData({
                            huibaomingcheng: obj[o],
        })
        ro.huibaomingcheng = true;
    continue;
}else{
    }
...
if(o=='wanchengqingkuang'){
            this.setData({
                            wanchengqingkuang: obj[o],
        })
        ro.wanchengqingkuang = true;
    continue;
}else{
    }
...
detailList,
        ...
        huibaomingcheng: data.huibaomingcheng,
        riqi:utils.getCurrentDate("yMDhms"),
        wanchengqingkuang: data.wanchengqingkuang,
        huibaoneirong: data.huibaoneirong,
        sfsh: '待审核',

        shhf: data.shhf,
}
this.data.editStatus? getApp().globalData.editorContent=data.huibaoneirong:""
...
if(this.data.riqi?.includes("请选择") || this.data.riqi==""){
wx.showToast({
icon: "none",
title: `日期不能为空`,
})
...
huibaomingcheng: this.data. huibaomingcheng,
riqi: this.data. riqi,
wanchengqingkuang: this.data. wanchengqingkuang,
huibaoneirong: getApp().globalData.editorContent,
sfsh:"是",
}
const detailId= getApp().globalData.detailId
const tableName= `gongzuohuibao`
...
let huibaoneirong=  getApp().globalData.editorContent
ruleForm['huibaoneirong']=getApp().globalData.editorContent
if(huibaoneirong==='' ){
wx.showToast({
icon: "none",
title: `汇报内容不能为空`,
...
let corssRes = await list(`gongzuohuibao`, params)
crossoptnum = wx.getStorageSync('statusColumnName');
...
if (ruleForm.id ) {
await update(`gongzuohuibao`, ruleForm)
}
else {
await add(`gongzuohuibao`, ruleForm)
}
}
